...
2026 RSNA 膝关节 MRI 异常检测比赛的可复现实验项目。当前已完成**阶段 0：准备环境与理解比赛**，正在进行**阶段 1：CSV 探索分析**。
...
- [ ] 分析每病例序列数和 Sagittal、Coronal、Axial 覆盖情况
- [ ] 分析 `Anatomical_Plane × Fluid_Sensitive × Fat_Suppression`
- [ ] 比较人工标注病例与无人工标签病例
- [ ] 分析报告长度、语言、重复和常见表达
- [ ] 汇总5–10条会影响验证、采样或建模的结论
...
继续完善 `notebooks/01_eda.ipynb`：分析每病例序列数，以及 Sagittal、Coronal、Axial 三个解剖平面的病例覆盖率。在阶段 1 的 CSV 结论完成前，不下载全部 DICOM。
